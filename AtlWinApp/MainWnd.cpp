...
// 파일 → 종료
LRESULT CMainWnd::OnFileExit(WORD, WORD, HWND, BOOL&)
{
...
LRESULT CMainWnd::OnPaint(UINT, WPARAM, LPARAM, BOOL&)
{
    ...
    // 3) 현재 로드된 비트맵이 있으면 화면에 그림
    if (m_hBmp) {
        // 3-1) 메모리 DC 생성 (비트맵을 그리기 위한 임시 DC)
        HDC memDC = CreateCompatibleDC(hdc);

        // 3-2) 메모리 DC에 m_hBmp 비트맵 선택
        ...
        // 3-4) BitBlt() : 메모리 DC(memDC)에 있는 비트맵을
        //                 윈도우 DC(hdc)로 복사해서 화면에 출력
        BitBlt(hdc, 0, 0, bm.bmWidth, bm.bmHeight, memDC, 0, 0, SRCCOPY);

        // 3-5) 원래 비트맵을 복구하고 메모리 DC 해제
        SelectObject(memDC, hOld);
        DeleteDC(memDC);
    }

    // 4) EndPaint() : BeginPaint()와 반드시 짝을 맞춰 호출해야 함
